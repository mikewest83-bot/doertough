...
function App() {
  ...
  const ensureMicPermission = async () => {
    ...
    try {
      ...
      if (err?.name === 'NotAllowedError' || err?.name === 'SecurityError') throw new Error('Safari is blocking microphone access for Mike. Set Microphone to Allow for doertoughmikeai.com, then reopen the site.');
  ...
  const startListening = async () => {
    ...
    if (!SpeechRecognition) { document.querySelector('#input')?.focus(); setError('Voice input is not supported by this browser. Type below to talk with Mike.'); return; }
    unlockAudio(); setError('');
    try { await ensureMicPermission(); } catch (err) { setStatus('ready'); setConversation(false); setError(err.message || 'Microphone access failed.'); return; }
    ...
    recognition.onerror = (e) => {
      setStatus('ready');
      if (e.error === 'not-allowed' || e.error === 'service-not-allowed') { setConversation(false); setError('Safari granted microphone access, but its speech-recognition service rejected the request. Try Safari again or type below.'); }
      else if (e.error === 'audio-capture') setError('Mike could not capture the microphone. Check that no other app is using it.');
      else if (e.error !== 'aborted' && e.error !== 'no-speech') setError(`Mike could not hear that (${e.error}). Try again.`);
    };
    recognition.onend = () => { if (statusRef.current === 'listening') setStatus('ready'); };
    try { recognition.start(); } catch (err) { setStatus('ready'); if (err.name !== 'InvalidStateError') setError('Mike could not start listening. Try again.'); }
  };

  const toggleConversation = () => {
    unlockAudio();
    if (conversationModeRef.current) { setConversation(false); recognitionRef.current?.abort(); stopSpeaking(); setStatus('ready'); return; }
    setConversation(true); stopSpeaking(); startListening();
  };

  const speak = async (text) => {
    stopSpeaking();
    const job = ++speakJobRef.current;
    setStatus('talking'); setError('');
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        Promise.resolve(el.play()).catch((err) => finish(reject, err));
      });
  ...
  const submitAuth = async (e) => {
    e?.preventDefault?.(); if (authBusy) return; setAuthBusy(true); setAuthError('');
    const path = authMode === 'login' ? '/api/auth/login' : '/api/auth/register';
    const body = authMode === 'login' ? { email: authForm.email, password: authForm.password } : authForm;
    try { const data = await fetchJson(path, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }, 20000); writeToken(data.token); setUser(data.user); setAuthOpen(false); setAuthForm({ name: '', email: '', password: '' }); }
    catch (err) { setAuthError(err.message || 'That did not work. Try again.'); }
    finally { setAuthBusy(false); }
  };

  const signOut = () => { writeToken(''); setUser(null); setMessages([{ role: 'mike', text: "Signed out. I'm still here if you want to talk." }]); };

  const ask = async (raw) => {
    const text = (raw || '').trim(); if (!text || busy) return;
    unlockAudio(); stopSpeaking(); setInput(''); setBusy(true); setError('');
    ...
    try {
      ...
      setBusy(false); await speak(data.text);
      if (conversationModeRef.current) setTimeout(() => { if (conversationModeRef.current && !busy) startListening(); }, 120);
    } catch (err) {
  ...
  return (
    <main>
      ...
      {authOpen && (
        <div className="auth-overlay" role="dialog" aria-modal="true" onClick={(e) => { if (e.target === e.currentTarget) setAuthOpen(false); }}>
          <div className="auth-card">
            ...
            <p className="auth-sub">{authMode === 'login' ? 'Sign in and Mike picks up where you left off.' : 'So Mike remembers you and your conversations stay yours.'}</p>
            <form onSubmit={submitAuth} className="auth-form">
              {authMode === 'register' && (<input placeholder="Your name" value={authForm.name} autoComplete="name" onChange={(e) => setAuthForm({ ...authForm, name: e.target.value })} />)}
              <input type="email" placeholder="Email" value={authForm.email} autoComplete="email" onChange={(e) => setAuthForm({ ...authForm, email: e.target.value })} />
              <input type="password" placeholder={authMode === 'login' ? 'Password' : 'Password (8+ characters)'} value={authForm.password} autoComplete={authMode === 'login' ? 'current-password' : 'new-password'} onChange={(e) => setAuthForm({ ...authForm, password: e.target.value })} />
              {authError && <div className="auth-error">{authError}</div>}
              <button type="submit" disabled={authBusy}>{authBusy ? 'Working...' : authMode === 'login' ? 'Sign in' : 'Create account'}</button>
            </form>
            <button className="auth-switch" onClick={() => { setAuthMode(authMode === 'login' ? 'register' : 'login'); setAuthError(''); }}>{authMode === 'login' ? "No account yet? Make one." : 'Already have an account? Sign in.'}</button>
          </div>
        </div>
      )}
      ...
      <section className="voice-hero">
        ...
        <button className={'talk voice-talk ' + (conversationMode ? 'active' : '')} onClick={toggleConversation} disabled={busy && !conversationMode}><Mic size={18} /> {conversationMode ? 'Stop Talking' : 'Talk to Mike'} <ArrowRight size={16} /></button>
        <div className="copy"><label>YOUR EVERYDAY COPILOT</label><h1>Meet Mike.<br /><span>Just talk.</span></h1><p>No avatar. No gimmicks. Just Mike. Talk it out, think it through, find the deal, make the move. Mike helps you research, plan, negotiate, write, buy, sell, and figure out what to do next.</p><button className="radar" onClick={() => ask('What am I missing?')} disabled={busy}><Lightbulb size={17} /> What am I missing?</button></div>
      </section>
