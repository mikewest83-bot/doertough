...
const CONTROL_DETAILS = {
  users: {
    ...
    description: 'Review account roles and access status.',
    rows: ['Total user accounts', 'Owner accounts', 'Admin accounts'],
  },
  audit: {
    title: 'Audit Activity',
    description: 'Review the latest security and owner events.',
    rows: ['Recent owner events', 'Authentication activity', 'Administrative actions'],
  },
  access: {
    title: 'Access Control',
    description: 'Review RBAC and protected owner capabilities.',
    rows: ['Owner-only routes', 'Owner-only tools', 'RBAC enforcement'],
  },
};
...
export default function OwnerPortal({ onClose }) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch('/api/owner/overview', { headers: authHeaders() })
      .then(async (res) => {
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
          const err = new Error(data.error || 'Owner dashboard unavailable.');
  ...
  const detail = CONTROL_DETAILS[selectedControl];
  ...
  return (
    <div className="owner-overlay" role="dialog" aria-modal="true" aria-label="Owner Access">
      <style>{`
        ...
        .owner-brand{display:flex;align-items:center;gap:12px}.owner-brand strong{display:block;font-size:14px;letter-spacing:.18em}.owner-brand small{display:block;margin-top:3px;color:#747c84;font-size:9px;letter-spacing:.2em}
        .owner-brand .brand-dt{width:48px!important;height:48px!important;flex-basis:48px!important}.owner-actions{display:flex;align-items:center;gap:9px}.owner-secure{padding:9px 12px;border:1px solid #f26b2166;border-radius:999px;color:#ff8a3d;background:#f26b210c;font-size:10px;font-weight:800}.owner-close{display:inline-flex;align-items:center;gap:6px;padding:9px 13px;border:1px solid #ffffff20;border-radius:999px;background:#ffffff06;color:#fff;font-weight:700;cursor:pointer}
        ...
        .owner-grid{display:grid;grid-template-columns:1.4fr 1fr;gap:12px;padding:0 28px 14px}.owner-panel{border:1px solid #ffffff12;border-radius:15px;background:#0d1217;overflow:hidden}.owner-panel-head{padding:16px 18px;border-bottom:1px solid #ffffff0d}.owner-panel-head h3{margin:0;font-size:17px}.owner-panel-head p{margin:4px 0 0;color:#6e7880;font-size:11px}.owner-event{display:grid;grid-template-columns:8px 1fr auto;gap:12px;align-items:center;padding:13px 18px;border-bottom:1px solid #ffffff09}.owner-event:last-child{border-bottom:0}.owner-event i{width:8px;height:8px;border-radius:50%;background:#ff7b27}.owner-event strong{display:block;font-size:12px}.owner-event small{display:block;margin-top:3px;color:#6e7880;font-size:10px}.owner-event time{color:#69737b;font-size:9px}
        .owner-quick{padding:10px}.owner-control{display:flex;align-items:center;gap:10px;width:100%;padding:14px 11px;margin-bottom:7px;border:1px solid #ffffff0c;border-radius:10px;background:#ffffff03;color:#fff;text-align:left;cursor:pointer}.owner-control:last-child{margin-bottom:0}.owner-control:hover,.owner-control:focus-visible{border-color:#ff7b2766;background:#ff7b2708;outline:none}.owner-control.active{border-color:#ff7b2777;background:#ff7b2710}.owner-quick span{color:#ff7b27;font-size:17px}.owner-quick strong{display:block;font-size:12px}.owner-quick small{display:block;margin-top:3px;color:#6d777f;font-size:10px}.owner-quick b{margin-left:auto;color:#68727a;font-size:20px;font-weight:400}
        .owner-detail{margin:0 28px 14px;padding:18px;border:1px solid #ff7b2733;border-radius:15px;background:#0e1318}.owner-detail h3{margin:0;font-size:16px}.owner-detail p{margin:5px 0 12px;color:#7d878f;font-size:11px}.owner-detail-list{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.owner-detail-item{padding:11px;border:1px solid #ffffff0c;border-radius:9px;background:#ffffff03;color:#aeb5bb;font-size:11px}.owner-detail-item b{display:block;margin-top:5px;color:#2bd866;font-size:10px}
        .owner-status{margin:0 28px 14px}.owner-status-row{display:grid;grid-template-columns:repeat(3,1fr)}.owner-status-row div{padding:15px 18px;border-right:1px solid #ffffff09;font-size:12px}.owner-status-row div:last-child{border-right:0}.owner-status-row b{float:right;color:#2bd866}.owner-footer{margin:0 28px 24px;padding:12px;border:1px solid #ffffff0c;border-radius:10px;text-align:center;color:#69737b;font-size:10px}
        @media(max-width:760px){.owner-overlay{padding:0 7px 20px}.owner-dashboard{border-radius:18px}.owner-topbar{padding:13px}.owner-secure{display:none}.owner-close{padding:8px 10px}.owner-hero{padding:22px 15px 15px;display:block}.owner-hero h2{font-size:35px}.owner-health{margin-top:14px;justify-content:center}.owner-stats{grid-template-columns:1fr 1fr;padding:0 15px 12px;gap:8px}.owner-stat{padding:14px}.owner-stat strong{font-size:27px}.owner-grid{grid-template-columns:1fr;padding:0 15px 12px}.owner-detail{margin:0 15px 12px}.owner-detail-list{grid-template-columns:1fr}.owner-status{margin:0 15px 12px}.owner-status-row{grid-template-columns:1fr}.owner-status-row div{border-right:0;border-bottom:1px solid #ffffff09}.owner-status-row div:last-child{border-bottom:0}.owner-footer{margin:0 15px 18px}.owner-event{grid-template-columns:8px 1fr}.owner-event time{display:none}}
      `}</style>
      <div className="owner-dashboard">
        <div className="owner-topbar">
          <div className="owner-brand"><b className="brand-dt"><span>D</span><em>T</em></b><div><strong>MIKE AI</strong><small>DOER TOUGH</small></div></div>
          <div className="owner-actions"><span className="owner-secure">🔒 OWNER ACCESS</span><button className="owner-close" onClick={onClose}><ArrowRight size={17} style={{transform:'rotate(180deg)'}} /> Mike</button></div>
        </div>
        ...
        {!loading && !error && overview && <>
          ...
          <div className="owner-grid"><section className="owner-panel"><div className="owner-panel-head"><h3>Recent Activity</h3><p>Latest security and owner events</p></div>{audit.length ? audit.map((item,index)=><div className="owner-event" key={`${item.createdAt||''}-${item.action||''}-${index}`}><i/><div><strong>{item.action}</strong><small>{item.targetType || 'System activity'}{item.targetId ? ` · ${item.targetId}` : ''}</small></div><time>{item.createdAt ? new Date(item.createdAt).toLocaleString() : ''}</time></div>) : <div className="owner-message">No audit activity recorded yet.</div>}</section><section className="owner-panel"><div className="owner-panel-head"><h3>Owner Controls</h3><p>Administrative areas</p></div><div className="owner-quick">
            <button type="button" className={`owner-control ${selectedControl === 'users' ? 'active' : ''}`} onClick={() => setSelectedControl('users')}><span>♟</span><div><strong>User Management</strong><small>Role and access controls</small></div><b>›</b></button>
            <button type="button" className={`owner-control ${selectedControl === 'audit' ? 'active' : ''}`} onClick={() => setSelectedControl('audit')}><span>▣</span><div><strong>Audit Activity</strong><small>Review security events</small></div><b>›</b></button>
            <button type="button" className={`owner-control ${selectedControl === 'access' ? 'active' : ''}`} onClick={() => setSelectedControl('access')}><span>✓</span><div><strong>Access Control</strong><small>RBAC status and permissions</small></div><b>›</b></button>
          </div></section></div>
          {detail && <section className="owner-detail" aria-live="polite"><h3>{detail.title}</h3><p>{detail.description}</p><div className="owner-detail-list">{detail.rows.map((row) => <div className="owner-detail-item" key={row}>{row}<b>Available</b></div>)}</div></section>}
