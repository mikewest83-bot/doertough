...
export function normalizeVisionImage(value) {
  ...
  if (!dataUrl || !ALLOWED_TYPES.has(mediaType)) {
    const error = new Error('vision_image_type_invalid'); error.status = 400; throw error;
  }
  const prefix = `data:${mediaType};base64,`;
  if (!dataUrl.startsWith(prefix)) {
    const error = new Error('vision_image_encoding_invalid'); error.status = 400; throw error;
  }
  if (dataUrl.length > MAX_DATA_URL_CHARS) {
    const error = new Error('vision_image_too_large'); error.status = 413; throw error;
  }
