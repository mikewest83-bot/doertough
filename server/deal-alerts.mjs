...
export async function cancelDealAlertTool(userId, args = {}) {
  try { const id = Number(args.id); if (!Number.isInteger(id) || id <= 0) return { error: 'deal_alert_id_invalid' }; return { tool: 'cancel_deal_alert', canceled: await cancelDealAlert(userId, id) }; }
  catch (error) { return toolError(error); }
}
