...
import { noteSubscriptionForReferral, rejectReferralForCustomer, rejectReferralForUser } from './mike-months.mjs';
...
async function applySubscription(user, subscription) {
  ...
  if (PRO_STATUSES.has(status)) {
    try {
      const referral = await noteSubscriptionForReferral(user.id, subscription);
      if (referral) console.log(`[mike-months] referral #${referral.id} subscription captured; reward eligible after ${referral.refund_eligible_at.toISOString()}`);
    } catch (error) {
      console.error('[mike-months] referral subscription capture failed:', error.message || error);
    }
  }
...
async function processStripeWebhookEvent(event) {
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      if (!userId) {
        console.error(`[stripe] checkout ${session.id} has no client_reference_id - cannot match it to an account`);
        return;
      }
      ...
      if (!session.customer || !session.subscription) {
        console.error(`[stripe] checkout ${session.id} is missing customer or subscription - cannot establish paid entitlement`);
        return;
      }
      ...
      if (!subscription) {
        console.error(`[stripe] checkout ${session.id} subscription ${session.subscription} could not be verified - entitlement unchanged`);
        return;
      }
    ...
    case 'customer.subscription.deleted': {
      ...
      if (!user) {
        console.error(`[stripe] subscription ${subscription.id} has no matching account (customer ${subscription.customer})`);
        return;
      }
      if (event.type === 'customer.subscription.deleted') {
        ...
        await rejectReferralForUser(user.id, 'subscription_canceled_during_refund_window');
      ...
      if (event.type === 'customer.subscription.updated' && (subscription.cancel_at_period_end || subscription.cancel_at)) {
        const rejected = await rejectReferralForUser(user.id, 'subscription_canceled_during_refund_window');
        if (rejected) console.log(`[mike-months] rejected ${rejected} referral(s) after subscription cancellation request for account #${user.id}`);
      }
    ...
    case 'charge.refunded': {
      ...
      if (charge.customer) {
        const rejected = await rejectReferralForCustomer(charge.customer, 'subscription_refunded_during_refund_window');
        if (rejected) console.log(`[mike-months] rejected ${rejected} referral(s) after refund for customer ${charge.customer}`);
      }
      return;
    }
    default:
