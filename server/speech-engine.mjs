...
async function createMikeGeneratedVoice() {
  ...
  const createResponse = await fetch('https://api.elevenlabs.io/v1/text-to-voice', { method: 'POST', headers: { 'xi-api-key': process.env.ELEVENLABS_API_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ voice_name: MIKE_VOICE_NAME, voice_description: MIKE_VOICE_DESCRIPTION, generated_voice_id: generatedVoiceId, labels: { accent: 'Southern American English', gender: 'male', use_case: 'conversational', character: 'Mike AI', region: 'South Carolina / Georgia / Tennessee' } }) });
...
async function chooseGeneratedVoice() {
  ...
  const data = await response.json(); const voices = data.voices || [];
  const exact = voices.find((voice) => voice.name === MIKE_VOICE_NAME); if (exact?.voice_id) return exact.voice_id;
...
async function respondToTranscript(transcript, signal, session) {
  requireKey(process.env.OPENAI_API_KEY, 'openai'); if (!openai) throw new Error('openai_client_missing');
  ...
  let input = transcriptToInput(transcript); let text = '';
  for (let round = 0; round < 4; round += 1) {
    ...
    console.log(`[speech-engine] voice tool round ${round + 1}: ${calls.map((c) => c.name).join(', ')}`); input = [...input, ...response.output];
...
export async function initializeSpeechEngine(httpServer) {
  ...
  const engineId = await ensureEngine(); const engine = await elevenlabs.speechEngine.get(engineId);
  await engine.attach(httpServer, WS_PATH, { debug: true,
    onInit: (conversationId) => console.log(`[speech-engine] session ${conversationId}`),
    onTranscript: async (transcript, signal, session) => { try { await respondToTranscript(transcript, signal, session); } catch (error) { if (error?.name === 'AbortError') return; console.error('[speech-engine] response failed:', error?.message || error); try { await session.sendResponse('I hit a snag for a second. Give me that one more time.'); } catch (sendError) { console.error('[speech-engine] fallback response failed:', sendError?.message || sendError); } } },
  ...
  console.log(`[speech-engine] attached at ${ENGINE_WS_URL}`); return engineId;
}
export async function getSpeechEngineToken() { requireKey(process.env.ELEVENLABS_API_KEY, 'elevenlabs'); const engineId = await ensureEngine(); const result = await elevenlabs.conversationalAi.conversations.getWebrtcToken({ agentId: engineId }); return { token: result.token, agentId: engineId }; }
export { ENGINE_NAME };
