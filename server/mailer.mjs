...
export async function sendPasswordReset({ to, name, resetUrl, expiresMinutes }) {
  ...
  const text = [
    greeting, '', 'Someone asked to reset the password on your Mike AI account.', '', resetUrl, '',
    ...
    'and your current password still works.', '', '- Doer Tough',
  ].join('\n');
  const html = `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:520px;color:#111">
      <p>${greeting}</p><p>Someone asked to reset the password on your Mike AI account.</p>
      <p style="margin:28px 0"><a href="${resetUrl}" style="background:#FF7611;color:#fff;padding:12px 22px;border-radius:6px;text-decoration:none;font-weight:600;display:inline-block">Reset your password</a></p>
...
export async function sendReminder({ to, name, title, note, remindAt }) {
  ...
  const html = `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:520px;color:#111">
      ...
      <h2 style="margin:12px 0 6px">${escapeHtml(safeTitle)}</h2>
      <p style="font-size:14px;color:#555">${escapeHtml(when)}</p>
      ${safeNote ? `<p>${escapeHtml(safeNote).replace(/\n/g, '<br>')}</p>` : ''}
