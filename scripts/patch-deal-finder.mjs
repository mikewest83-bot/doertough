...
patchOnce(
  ...
  '  handlers: { ...LIVE_TOOL_HANDLERS, set_deal_alert: (args, user) => dealAlertHandlerFor(\'set_deal_alert\', user?.id)?.(args), list_deal_alerts: (_args, user) => dealAlertHandlerFor(\'list_deal_alerts\', user?.id)?.(), cancel_deal_alert: (args, user) => dealAlertHandlerFor(\'cancel_deal_alert\', user?.id)?.(args) },',
