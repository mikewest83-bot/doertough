...
patchOnce(
  ...
  'const LIVE_TOOLS = [...BASE_TOOLS, ...BUSINESS_TOOLS, ...FREE_TOOLS, ...FIELD_TOOLS, ...DEAL_FINDER_TOOLS, ...DEAL_ALERT_TOOLS];',
  'Deal Finder tools'
);
...
patchOnce(
  ...
  "  handlers: { ...LIVE_TOOL_HANDLERS, set_deal_alert: (input) => dealAlertHandlerFor('set_deal_alert', input?.user?.id)?.(input), list_deal_alerts: (input) => dealAlertHandlerFor('list_deal_alerts', input?.user?.id)?.(), cancel_deal_alert: (input) => dealAlertHandlerFor('cancel_deal_alert', input?.user?.id)?.(input) },",
  'Deal Alert gateway handlers'
);
