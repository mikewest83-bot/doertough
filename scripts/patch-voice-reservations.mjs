...
if (!source.includes('const reserveResult = await reserveVoiceSession({')) {
  ...
  if (startIndex < 0) throw new Error('Voice token reservation start anchor not found');

  // The route exists in two known source shapes: the legacy multiline response
  // and the current compact response. Match the end independently instead of
  // assuming the presence of the legacy start implies a legacy end.
  const legacyEndIndex = source.indexOf(legacyEnd, startIndex);
  const hardenedEndIndex = source.indexOf(hardenedEnd, startIndex);
  const endIndex = legacyEndIndex >= 0 ? legacyEndIndex : hardenedEndIndex;
  const endLength = legacyEndIndex >= 0 ? legacyEnd.length : hardenedEnd.length;
  if (endIndex < 0) throw new Error('Voice token reservation response anchor not found');
  ...
  source = source.slice(0, startIndex) + replacement + source.slice(endIndex + endLength);
}
